		...
		if(!empty($itemNumber) && isset($customerID) && isset($saleDate) && isset($quantity) && isset($unitPrice)){
			...
			if($stockStatement->rowCount() > 0){
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Inventory is empty. Therefore, can\'t make a Dispatch. Please select a different Product.</div>';
					exit();
				} elseif ($currentQuantityInItemsTable < $quantity) {
					// Requested sale quantity is higher than available item quantity. Hence abort 
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Not enough inventory available for this dispatch. Therefore, can\'t make a Dispatch. Please select a different Product.</div>';
					exit();
				}
				else {
					...
					if($customerStatement->rowCount() > 0){
						...
						echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Dispatch details added to DB and stocks updated.</div>';
						exit();
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Item already exists in DB. Please click the <strong>Update</strong> button to update the details. Or use a different Product Number.</div>';
				exit();
			} else {
