	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
			...
			if($saleDetailsSaleID == ''){ 
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter a Dispatch ID.</div>';
				exit();
			}
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					if($stockStatement->rowCount() > 0){
						...
						echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Dispatch details updated.</div>';
						exit();
						
					} else {
						// Item does not exist in item table, therefore, you can't update 
						// sale details for it 
						echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Product does not exist in DB. Therefore, first enter this Product to DB using the <strong>Item</strong> tab.</div>';
						exit();
					}	
				...
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Dispatch details does not exist in DB for the given Dispatch ID. Therefore, can\'t update.</div>';
				exit();
